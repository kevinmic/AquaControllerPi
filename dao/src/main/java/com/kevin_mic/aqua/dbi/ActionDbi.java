...
public interface ActionDbi {
    @SqlUpdate("insert into " + ActionDB.TABLE_NAME + " (actionId, name, actionType, actionJson) values (:actionId, :name, :actionType, :actionJson)")
    void insertAction(@BindBean ActionDB action);

    @SqlUpdate("update " + ActionDB.TABLE_NAME + " set name = :name, actionJson = :actionJson where actionId = :actionId")
