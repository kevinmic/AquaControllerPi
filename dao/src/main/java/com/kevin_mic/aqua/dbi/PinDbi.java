...
public abstract class PinDbi {
    ...
    @SqlUpdate("insert into " + Pin.TABLE_NAME + " (pinId, pinSupplierId) values (:pinId, :pinSupplierId)")
    public abstract void insert(@BindBean Pin pin);

    public void addDeviceOwnershipWithCheck(@Bind("pinId") String pinId, @Bind("deviceId") int deviceId) {
    ...
    public abstract Pin getPin_ForUpdate(@Bind("pinId") String pinId);

    @SqlUpdate("update " + Pin.TABLE_NAME + " set ownedByDeviceId = :deviceId where pinId = :pinId")
    abstract void addDeviceOwnership(@Bind("pinId") String pinId, @Bind("deviceId") int deviceId);

    @SqlUpdate("update " + Pin.TABLE_NAME + " set ownedByDeviceId = null where pinId = :pinId and ownedByDeviceId = :deviceId")
    public abstract void removeDeviceOwnership(@Bind("pinId") String pinId, @Bind("deviceId") int deviceId);
